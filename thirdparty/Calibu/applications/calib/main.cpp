...
int main(int argc, char** argv) {
    ...
    cl.disable_loop();
    cl.reset_cursor();
    for (std::string filename = cl.follow("", 2, "-cameras", "-c"); !filename.empty(); filename = cl.follow("", 2, "-cameras", "-c")) {
    ...
    for (size_t i = 0; i < N; ++i) {
        ...
        if (i < input_cameras.size()) {
            calib_cams[i] = calibrator.AddCamera(input_cameras[i].camera, input_cameras[i].T_ck);
        } else {
            ...
            size_ << w_i, h_i;
            params_ << 300, 300, w_i / 2.0, h_i / 2.0, 0.2;
            std::shared_ptr<FovCamera<double>> starting_cam(new FovCamera<double>(params_, size_));
    ...
    if (gui) {
        ...
        pangolin::View& container
            = pangolin::CreateDisplay().SetBounds(1.0, 0.0, pangolin::Attach::Pix(PANEL_WIDTH), 1.0).SetLayout(pangolin::LayoutEqual);
...
// OpenGl Texture for video frame
#ifdef _GCC_
        // MSVC / Clang doesn't support non-compile-time-staticly sized non-pod arrays
        ...
        for (int frame = 0; !pangolin::ShouldQuit();) {
            ...
            for (size_t iI = 0; iI < N; ++iI) {
                ...
                const std::vector<Conic, Eigen::aligned_allocator<Conic>>& conics = conic_finder.Conics();
                ...
                if (tracking_good[iI]) {
                    ...
                    std::vector<Eigen::Vector2d, Eigen::aligned_allocator<Eigen::Vector2d>> ellipses;
                    for (size_t i = 0; i < conics.size(); ++i) {
                        ellipses.push_back(conics[i].center);
                    }

                    // find camera pose given intrinsics
                    PosePnPRansac(calibrator.GetCamera(iI).camera, ellipses, target.Circles3D(), ellipse_target_map, 0, 0, &T_hw[iI]);
                ...
                if (container[iI].IsShown()) {
                    ...
                    if (tracking_good[iI] && disp_barcode) {
                        const std::vector<Eigen::Vector3d, Eigen::aligned_allocator<Eigen::Vector3d>>& codepts = target.Code3D();
                        ...
                        for (size_t c = 0; c < codepts.size(); c++) {
                            ...
                            if (pt[0] < 10 || pt[0] >= images[iI].w - 10 || pt[1] < 10 || pt[1] >= images[iI].h - 10) {
                                found = false;
                                continue;
                            }
        ...
        while (valid_frame) {
            ...
            int calib_frame = calibrator.AddFrame(Sophus::SE3d(Sophus::SO3d(), Eigen::Vector3d(0, 0, 1000)));
