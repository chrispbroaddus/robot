...
public:
    ...
    bool ProcessFrame(std::shared_ptr<CameraInterface<double>> cam, const unsigned char* I, size_t w, size_t h, size_t pitch);

    const TargetInterface& Target() const { return target; }

    const ConicFinder& GetConicFinder() const { return conic_finder; }

    const ImageProcessing& Images() const { return imgs; }

    const std::vector<int>& ConicsTargetMap() const { return conics_target_map; }

    const Sophus::SE3d& PoseT_gw() const { return T_gw; }
