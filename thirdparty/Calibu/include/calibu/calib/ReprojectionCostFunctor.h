...
template <typename CameraInt> struct ReprojectionCostFunctor {
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW;
    ReprojectionCostFunctor(const Eigen::Vector3d& Pw, const Eigen::Vector2d& pc)
        : m_Pw(Pw)
        , m_pc(pc) {}

    template <typename T> bool operator()(const T* const pT_kw, const T* const pT_ck, const T* const camparam, T* residuals) const {
