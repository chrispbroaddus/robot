...
template <int P> struct SumParams<P> { static const int value = P; };

template <int P, int... PS> struct SumParams<P, PS...> { static const int value = P + SumParams<PS...>::value; };

// Sum first N variadic compile time values
template <int... ALL> struct SumParamsN;

template <int N, int P, int... PS> struct SumParamsN<N, P, PS...> {
    static const int value = P + std::conditional<N == 1, SumParams<0>, SumParamsN<N - 1, PS...>>::value;
};

// Take Nth variadic compile time value
template <int... ALL> struct ParamsN;

template <int N, int P, int... PS> struct ParamsN<N, P, PS...> {
    static const int value = std::conditional<N == 0, SumParams<P>, ParamsN<N - 1, PS...>>::value;
};

template <typename JetT, typename T, unsigned int Start, unsigned int... Blocks> struct Take1stOrderPart;

template <typename JetT, typename T, unsigned int Start> struct Take1stOrderPart<JetT, T, Start> {
...
class AutoDiffArrayCostFunction : public CostBase {
public:
    ...
    virtual bool Evaluate(T const* const* parameters, T* residuals, T** jacobians) const {
        if (!jacobians) {
