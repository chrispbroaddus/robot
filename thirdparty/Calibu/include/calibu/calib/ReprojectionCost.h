...
struct ReprojectionCost : public ceres::AutoDiffArrayCostFunction<ceres::CostFunction, ReprojectionCost<ProjModel>, 2,
                              Sophus::SE3d::num_parameters, Sophus::SE3d::num_parameters, ProjModel::NUM_PARAMS> {
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW;
    ReprojectionCost(const Eigen::Vector3d& Pw, const Eigen::Vector2d& pc)
        : m_Pw(Pw)
        , m_pc(pc) {}

    template <typename T = double> bool Evaluate(T const* const* parameters, T* residuals) const {
