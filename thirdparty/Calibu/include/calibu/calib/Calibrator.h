...
struct CameraAndPose {
    CameraAndPose(const std::shared_ptr<CameraInterface<double>> camera, const Sophus::SE3d& T_ck)
        : camera(camera)
        , T_ck(T_ck) {}
...
public:
    ...
    int AddCamera(const std::shared_ptr<CameraInterface<double>> cam, const Sophus::SE3d& T_ck = Sophus::SE3d()) {
    ...
    /// minimization.
    void FixCameraIntrinsics(bool v = true) { m_fix_intrinsics = v; }
    ...
    void AddObservation(size_t frame, size_t camera, const Eigen::Vector3d& P_w, const Eigen::Vector2d& p_c) {
        m_update_mutex.lock();
        ...
        if (dynamic_cast<FovCamera<double>*>(interface.get())) {
            cost->Cost() = new ceres::AutoDiffCostFunction<ReprojectionCostFunctor<FovCamera<double>>, 2, Sophus::SE3d::num_parameters,
                Sophus::SE3d::num_parameters, calibu::FovCamera<double>::NumParams>(
                new ReprojectionCostFunctor<FovCamera<double>>(P_w, p_c));
        } else if (dynamic_cast<Poly2Camera<double>*>(interface.get())) {
            cost->Cost() = new ceres::AutoDiffCostFunction<ReprojectionCostFunctor<Poly2Camera<double>>, 2, Sophus::SE3d::num_parameters,
                Sophus::SE3d::num_parameters, calibu::Poly2Camera<double>::NumParams>(
                new ReprojectionCostFunctor<Poly2Camera<double>>(P_w, p_c));
        } else if (dynamic_cast<LinearCamera<double>*>(interface.get())) {
            cost->Cost() = new ceres::AutoDiffCostFunction<ReprojectionCostFunctor<LinearCamera<double>>, 2, Sophus::SE3d::num_parameters,
                Sophus::SE3d::num_parameters, calibu::LinearCamera<double>::NumParams>(
                new ReprojectionCostFunctor<LinearCamera<double>>(P_w, p_c));
        } else if (dynamic_cast<Poly3Camera<double>*>(interface.get())) {
            cost->Cost() = new ceres::AutoDiffCostFunction<ReprojectionCostFunctor<Poly3Camera<double>>, 2, Sophus::SE3d::num_parameters,
                Sophus::SE3d::num_parameters, calibu::Poly3Camera<double>::NumParams>(
                new ReprojectionCostFunctor<Poly3Camera<double>>(P_w, p_c));
        } else if (dynamic_cast<KannalaBrandtCamera<double>*>(interface.get())) {
            cost->Cost() = new ceres::AutoDiffCostFunction<ReprojectionCostFunctor<KannalaBrandtCamera<double>>, 2,
                Sophus::SE3d::num_parameters, Sophus::SE3d::num_parameters, calibu::KannalaBrandtCamera<double>::NumParams>(
                new ReprojectionCostFunctor<KannalaBrandtCamera<double>>(P_w, p_c));
        } else {
            throw std::runtime_error("Don't know how to optimize Camera.");
        }

        cost->Params() = std::vector<double*>{ T_kw.data(), cp.T_ck.data(), cp.camera->GetParams().data() };
    ...
    size_t NumFrames() const { return m_T_kw.size(); }

    /// Return pose of camera rig frame i
    Sophus::SE3d& GetFrame(size_t i) { return *m_T_kw[i]; }

    /// Return number of cameras in camera rig
    size_t NumCameras() const { return m_camera.size(); }

    /// Return camera i of camera rig
    CameraAndPose GetCamera(size_t i) { return *m_camera[i]; }

    /// Return current Mean Square reprojection Error - the objective function
    /// being minimised by optimisation.
    double MeanSquareError() const { return m_mse; }

    /// Return true if one of the tolerance criteria is reached.
    bool ReachedTolerance() { return ((m_termination_type == ceres::CONVERGENCE)); }
        ...
        while (m_should_run) {
            ...
            if (problem.NumResiduals() > 0) {
                try {
                    ...
                    std::cout << "Frames: " << m_T_kw.size() << "; Observations: " << summary.num_residuals << "; mse: " << m_mse
                              << std::endl;
