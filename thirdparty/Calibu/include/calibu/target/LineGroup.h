...
struct Triple {
    ...
    inline bool In(const std::set<Vertex*> bag) const { return bag.find(vs[0]) != bag.end() && bag.find(vs[2]) != bag.end(); }
...
inline bool operator==(const Vertex& lhs, const Vertex& rhs) { return lhs.id == rhs.id; }

inline bool AreCollinear(const Triple& t1, const Triple& t2) { return t1.Contains(t2.Center()) && t2.Contains(t1.Center()); }

inline double Distance(const Vertex& v1, const Vertex& v2) { return (v2.pc - v1.pc).norm(); }
