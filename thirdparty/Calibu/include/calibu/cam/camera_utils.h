...
struct CameraUtils {
    ...
    template <typename T> static inline T PixNorm(const T* pix) { return sqrt(pix[0] * pix[0] + pix[1] * pix[1]); }

    template <typename T> static void Scale(const double s, T* params) {
        params[0] *= s;
        params[1] *= s;
    ...
    template <typename T> static inline void K(const T* params, T* Kmat) {
        Kmat[0] = params[0];
        Kmat[1] = 0;
        Kmat[2] = 0;
        Kmat[3] = 0;
     ...
     * */
    template <typename T> static inline void Dehomogenize(const T* ray, T* px_dehomogenized) {
     ...
     */
    template <typename T> static inline void Homogenize(const T* pix, T* ray_homogenized) {
     ...
     */
    template <typename T> static inline void dDehomogenize_dray(const T* ray, T* j) {
    ...
    template <typename T> static inline void dMultK_dparams(const T*, const T* pix, T* j) {
        j[0] = pix[0];
        j[2] = 0;
        j[4] = 1;
        j[6] = 0;
    ...
    template <typename T> static inline void dMultInvK_dparams(const T* params, const T* pix, T* j) {
     ...
     */
    template <typename T> static inline void MultK(const T* params, const T* pix, T* pix_k) {
     ...
     */
    template <typename T> static inline void MultInvK(const T* params, const T* pix, T* pix_kinv) {
        pix_kinv[0] = (pix[0] - params[2]) / params[0];
        pix_kinv[1] = (pix[1] - params[3]) / params[1];
    }
};

template <typename Scalar = double> class LinearCamera : public CameraImpl<Scalar, 4, LinearCamera<Scalar>> {
