...
struct LookupTable {
    inline LookupTable()
        : m_vLutPixels(std::vector<BilinearLutPoint>())
        , m_nWidth(0){};
    ...
    inline unsigned int Width() const { return m_nWidth; }

    inline unsigned int Height() const { return m_vLutPixels.size() / m_nWidth; }
...
enum BorderTreatment { BORDER_REPEAT, BORDER_BLACK };
...
/// camera intrinsics K.
CALIBU_EXPORT void CreateLookupTable(const std::shared_ptr<calibu::CameraInterface<double>>& cam_from, const Eigen::Matrix3d& R_onKinv,
    LookupTable& lut, int lookup_width = 0, int lookup_height = 0);

/// Create lookup table which can be used to remap a general camera model
/// 'cam_from' to a linear.
void CreateLookupTable(
    const std::shared_ptr<calibu::CameraInterface<double>>& cam_from, LookupTable& lut, int lookup_width = 0, int lookup_height = 0);
...
CALIBU_EXPORT void Rectify(
    const LookupTable& lut, const scalar* pInputImageData, scalar* pOutputRectImageData, int w, int h, int channels = 1) {
    ...
    for (int nRow = 0; nRow < nHeight; nRow++) {
        for (int nCol = 0; nCol < nWidth; nCol++) {
            for (int n_channel = 0; n_channel < channels; ++n_channel) {
                *pOutputRectImageData++ = (scalar)(ptr->w00 * pInputImageData[ptr->idx0 * channels + n_channel]
                    + ptr->w01 * pInputImageData[(ptr->idx0 + 1) * channels + n_channel]
                    + ptr->w10 * pInputImageData[ptr->idx1 * channels + n_channel]
                    + ptr->w11 * pInputImageData[(ptr->idx1 + 1) * channels + n_channel]);
            }
            ptr++;
        }
    }
...
template <typename T> inline Eigen::Matrix<T, 2, 1> Project(const Eigen::Matrix<T, 3, 1>& P) {
    return Eigen::Matrix<T, 2, 1>(P(0) / P(2), P(1) / P(2));
}

template <typename T> inline Eigen::Matrix<T, 3, 1> Project(const Eigen::Matrix<T, 4, 1>& P) {
