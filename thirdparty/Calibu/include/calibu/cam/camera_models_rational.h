...
template <typename Scalar = double> class Rational6Camera : public CameraImpl<Scalar, 10, Rational6Camera<Scalar>> {
...
public:
    ...
    template <typename T> static T Factor(const T rad, const T* params) {
        ...
        return ((static_cast<T>(1.0) + params[4] * r2 + params[5] * r4 + params[6] * r4 * r2)
            / (static_cast<T>(1.0) + params[7] * r2 + params[8] * r4 + params[9] * r4 * r2));
    }

    template <typename T> static T dFactor_drad(const T ru, const T* params, T* fac) {
    ...
    template <typename T> static T Factor_inv(const T rd, const T* params) {
        T k1 = params[4];
        T k2 = params[5];
        T k3 = params[6];
        T k4 = params[7];
