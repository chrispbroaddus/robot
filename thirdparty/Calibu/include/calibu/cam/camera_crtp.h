...
template <typename Scalar = double> class CameraInterface {
protected:
...
public:
    ...
    virtual Eigen::Matrix<Scalar, 2, 3> dProject_dray(const Vec3t& ray) const = 0;

    virtual Eigen::Matrix<Scalar, 2, Eigen::Dynamic> dProject_dparams(const Vec3t& ray) const = 0;

    virtual Eigen::Matrix<Scalar, 3, Eigen::Dynamic> dUnproject_dparams(const Vec2t& pix) const = 0;
     ...
     */
    Vec2t Transfer3d(const SE3t& t_ba, const Vec3t& ray, const Scalar rho) const {
        const Vec3t ray_dehomogenized = t_ba.rotationMatrix() * ray + rho * t_ba.translation();
     ...
     * being transfered.
     */
    Eigen::Matrix<Scalar, 2, 4> dTransfer3d_dray(const SE3t& t_ba, const Vec3t& ray, const Scalar rho) const {
        const Eigen::Matrix<Scalar, 3, 3> rot_matrix = t_ba.rotationMatrix();
        const Vec3t ray_dehomogenized = rot_matrix * ray + rho * t_ba.translation();
        const Eigen::Matrix<Scalar, 2, 3> dproject_dray = dProject_dray(ray_dehomogenized);
        Eigen::Matrix<Scalar, 2, 4> dtransfer3d_dray;
        dtransfer3d_dray.template topLeftCorner<2, 3>() = dproject_dray * rot_matrix;
        dtransfer3d_dray.col(3) = dproject_dray * t_ba.translation();
        return dtransfer3d_dray;
    }

    Eigen::Matrix<Scalar, 2, Eigen::Dynamic> dTransfer_dparams(const SE3t& t_ba, const Vec2t& pix, const Scalar rho) const {
        const Vec3t ray = Unproject(pix);
        const Eigen::Matrix<Scalar, 3, 3> rot_matrix = t_ba.rotationMatrix();
        const Vec3t ray_dehomogenized = rot_matrix * ray + rho * t_ba.translation();
        const Eigen::Matrix<Scalar, 2, 3> dproject_dray = dProject_dray(ray_dehomogenized);
        const Eigen::Matrix<Scalar, 2, 3> dtransfer3d_dray = dproject_dray * rot_matrix;
        const Eigen::Matrix<Scalar, 3, Eigen::Dynamic> dray_dparams = dUnproject_dparams(pix);

        const Eigen::Matrix<Scalar, 2, Eigen::Dynamic> d_project_dparams = dProject_dparams(ray_dehomogenized);
    ...
    Eigen::Matrix<Scalar, 3, 3> RDF() const { return rdf_; }

    uint64_t SerialNumber() const { return serialNo_; }
    ...
    void SetName(const std::string& sName) { name_ = sName; }

    /// Set serial number of camera, if applicable.
    void SetSerialNumber(const uint64_t nSerialNo) { serialNo_ = nSerialNo; }

    /// Set index for multi-camera rigs.
    void SetIndex(const int nIndex) { index_ = nIndex; }

    /// Set version number of camera.
    void SetVersion(int nVersion) { version_ = nVersion; }
    ...
    void SetRDF(const Eigen::Matrix<Scalar, 3, 3>& sRDF) { rdf_ = sRDF; }

    /// Set generic camera intrinsics and parameters.
    void SetParams(const Eigen::VectorXd params) { params_ = params; }

    /// Set the pose of the camera (typically in the "rig" frame).
    void SetPose(const SE3t& t_rc) { t_rc_ = t_rc; }

    /// Return camera pose (typically in the "rig's" frame).
    SE3t Pose() const { return t_rc_; }
...
protected:
    ...
    CameraInterface(const Eigen::VectorXd& params_in, const Eigen::Vector2i& image_size)
...
template <typename Scalar = double> class Rig {
public:
    Rig() {}

    void AddCamera(std::shared_ptr<CameraInterface<Scalar>> cam) { cameras_.push_back(cam); }

    size_t NumCams() { return cameras_.size(); }
