...
Eigen::Matrix3d EstimateH_ba(const std::vector<Eigen::Vector2d, Eigen::aligned_allocator<Eigen::Vector2d>>& a,
    ...
    for (unsigned int i = 0; i < a.size(); ++i) {
        ...
        M.block<2, 9>(i * 2, 0) << u1, v1, 1, 0, 0, 0, -u1 * u2, -v1 * u2, -u2, 0, 0, 0, u1, v1, 1, -u1 * v2, -v1 * v2, -v2;
    }

    const Matrix<double, 9, 9> Vt = Eigen::JacobiSVD<MatrixXd>(M, ComputeFullV).matrixV().transpose();
