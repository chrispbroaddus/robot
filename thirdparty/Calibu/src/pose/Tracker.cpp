...
bool Tracker::ProcessFrame(std::shared_ptr<CameraInterface<double>> cam, const unsigned char* I, size_t w, size_t h, size_t pitch) {
    double rms = 0;
    ...
    conics_target_map = PosePnPRansac(
        cam, ellipses, target.Circles3D(), conics_candidate_map_first_pass, params.robust_3pt_its, params.robust_3pt_inlier_tol, &T_hw);
    ...
    conics_target_map = PosePnPRansac(
        cam, ellipses, target.Circles3D(), conics_candidate_map_second_pass, params.robust_3pt_its, params.robust_3pt_inlier_tol, &T_hw);
