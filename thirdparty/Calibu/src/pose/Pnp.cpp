...
vector<int> PosePnPRansac(const std::shared_ptr<CameraInterface<double>> cam,
    const std::vector<Eigen::Vector2d, Eigen::aligned_allocator<Eigen::Vector2d>>& img_pts,
    const std::vector<Eigen::Vector3d, Eigen::aligned_allocator<Eigen::Vector3d>>& ideal_pts, const vector<int>& candidate_map,
    int robust_3pt_its, float robust_3pt_tol, Sophus::SE3d* T) {
    ...
    if (robust_3pt_its > 0) {
        cv::solvePnPRansac(
            cv_obj, cv_img, cv_K, cv_coeff, cv_rot, cv_trans, false, robust_3pt_its, robust_3pt_tol / cam->K()(0, 0), 60, cv_inliers);
    } else {
...
double ReprojectionErrorRMS(const std::shared_ptr<CameraInterface<double>> cam, const Sophus::SE3d& T_cw,
    const std::vector<Eigen::Vector3d, Eigen::aligned_allocator<Eigen::Vector3d>>& pts3d,
    const std::vector<Eigen::Vector2d, Eigen::aligned_allocator<Eigen::Vector2d>>& pts2d, const vector<int>& map2d_3d) {
    int n = 0;
    double sse = 0;
