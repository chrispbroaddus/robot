...
Eigen::Matrix3d FindEllipse(const int w, const int /*h*/, const TdI* dI, const IRectangle& r, double& /*residual*/
    ) {
...
template <typename TdI>
void FindConics(const int w, const int h, const std::vector<PixelClass>& candidates, const TdI* dI,
...
void FindCandidateConicsFromLabels(unsigned w, unsigned h, const std::vector<PixelClass>& labels, std::vector<PixelClass>& candidates,
    float min_area, float max_area, float min_density, float min_aspect) {
    const int border = 3;
...
template void FindConics(const int, const int, const std::vector<PixelClass>& candidates, const Eigen::Vector2f* dI,
    std::vector<Conic, Eigen::aligned_allocator<Conic>>& conics);
}
