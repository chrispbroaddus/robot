...
Eigen::Vector2d GetAxesLengths(const Conic& c) {
    ...
    double det_ratio = -c.C.determinant() / c.C.topLeftCorner<2, 2>().determinant();
    return Eigen::Vector2d(sqrt(det_ratio / eigenval[0]), sqrt(det_ratio / eigenval[1]));
}
...
double Cost(const pair<Vector3d, Matrix3d>& plane, const Conic& conic, const Matrix3d& K) {
    ...
    return amc / max(abs(a), abs(c));
    ; // + b*b;
}

double Cost(const pair<Vector3d, Matrix3d>& plane, const vector<Conic, Eigen::aligned_allocator<Conic>>& conics, const Matrix3d& K,
    double threshold) {
    int n = 0;
    double sum = 0;
...
pair<Vector3d, Matrix3d> PlaneFromConics(
    const vector<Conic, Eigen::aligned_allocator<Conic>>& conics, double plane_circle_radius, const Matrix3d& K, double inlier_threshold) {
