...
void SaveEPS(std::string filename, const Eigen::MatrixXi& M, const Eigen::Vector2d& offset, double grid_spacing, double rad0, double rad1,
    double pts_per_unit, unsigned char id) {
    const double border = 3 * rad1;
    const Eigen::Vector2d border2d(border, border);
    const Eigen::Vector2d max_pts(
        pts_per_unit * ((M.cols() - 1) * grid_spacing + 2 * border), pts_per_unit * ((M.rows() - 1) * grid_spacing + 2 * border));
...
int NumExactMatches(
    const std::array<Eigen::MatrixXi, 4>& PG, const Eigen::MatrixXi& m, int& best_score, int& best_g, int& best_r, int& best_c) {
    best_score = std::numeric_limits<int>::max();
    int num_exact = 0;
...
int SeedScore(uint32_t seed, int r, int c) { return AutoCorrelationMinArea(MakePatternGroup(r, c, seed)); }
