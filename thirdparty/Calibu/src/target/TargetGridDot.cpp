...
TargetGridDot::TargetGridDot(const std::string& preset) {
    ...
    calibu::LoadGridFromPreset(preset, grid, grid_spacing_, large_dot_radius, small_dot_radius);
...
std::vector<std::vector<Dist>> ClosestPoints(std::vector<Vertex, Eigen::aligned_allocator<Vertex>>& pts) {
    std::vector<std::vector<Dist>> ret;

    // Set size of arrays
    ret.resize(pts.size());
    for (size_t p1 = 0; p1 < pts.size(); ++p1)
        ret[p1].resize(pts.size());
...
std::set<Vertex*> Neighbours(
    std::map<Eigen::Vector2i, Vertex*, std::less<Eigen::Vector2i>, Eigen::aligned_allocator<std::pair<Eigen::Vector2i, Vertex*>>>& map,
    const Vertex& v) {
    std::set<Vertex*> neighbours;
    for (int r = -1; r <= 1; ++r) {
        for (int c = -1; c <= 1; ++c) {
...
bool TargetGridDot::FindTarget(const Sophus::SE3d& T_cw, const std::shared_ptr<CameraInterface<double>> cam, const ImageProcessing& images,
    const std::vector<Conic, Eigen::aligned_allocator<Conic>>& conics, std::vector<int>& ellipse_target_map) {
    // This target doesn't use position or camera information
    return FindTarget(images, conics, ellipse_target_map);
}

bool TargetGridDot::FindTarget(const std::shared_ptr<CameraInterface<double>> cam, const ImageProcessing& images,
    const std::vector<Conic, Eigen::aligned_allocator<Conic>>& conics, std::vector<int>& ellipse_target_map) {
    // This target doesn't use position or camera information
    return FindTarget(images, conics, ellipse_target_map);
}
...
bool TargetGridDot::Match(
    std::map<Eigen::Vector2i, Vertex*, std::less<Eigen::Vector2i>, Eigen::aligned_allocator<std::pair<Eigen::Vector2i, Vertex*>>>& obs,
    ...
    if (osize[0] > 2 && osize[1] > 2) {
        ...
        {
            // Found unique match
            Sophus::SE2Group<int> T_0x[4] = { Sophus::SE2Group<int>(Sophus::SO2Group<int>(1, 0), Eigen::Vector2i(0, 0)),
...
bool TargetGridDot::FindTarget(const ImageProcessing& images, const std::vector<Conic, Eigen::aligned_allocator<Conic>>& conics,
...
void PlotCrossHair(double x, double y, double size, std::ofstream& f) {
    double l = x - size / 2;
    double r = x + size / 2;
    double t = y + size / 2;
    double b = y - size / 2;
...
void TargetGridDot::SaveEPS(
    std::string filename, const Eigen::Vector2d& offset, double rad0, double rad1, double pts_per_unit, unsigned char id) const {
    ...
    const Eigen::Vector2d max_pts(
        pts_per_unit * ((M.cols() - 1) * grid_spacing_ + 2 * border), pts_per_unit * ((M.rows() - 1) * grid_spacing_ + 2 * border));
    ...
    for (int r = 0; r < M.rows(); ++r) {
        for (int c = 0; c < M.cols(); ++c) {
            ...
            f << p_pts[0] << " " << max_pts[1] - p_pts[1] << " " << rad_pts << " 0 360 arc closepath" << std::endl
              << "0.0 setgray fill" << std::endl
              << std::endl;
        }
    }

    std::cerr << "Wrote bounding box: " << max_pts[0] << " " << max_pts[1] << std::endl;
    ...
    for (int c = 0; c < 8; c++) {
        if (id & 1 << c) {
            const Eigen::Vector2d p = pts_per_unit * (offset + base + border2d + Eigen::Vector2d(dx * c, r));
            f << "newpath\n"
    ...
    f << "newpath\n"
      ...
      << "(Gridspacing: " << grid_spacing_ << "cm) show\n";
...
void TargetGridDot::SaveSVG(std::string filename, double rad0, double rad1) const {
    Eigen::MatrixXi M = GetBinaryPattern(0);
    ...
    double canvas_height = ((M.rows() - 1) * grid_spacing_ * 1e3) + offset * 2. + margin_bottom;
    f << "<svg width=\"" << canvas_width << "mm\" height=\"" << canvas_height << "mm\">" << std::endl;
    ...
    for (int r = 0; r < M.rows(); ++r) {
        ...
        for (int c = 0; c < M.cols(); ++c) {
            ...
            f << "<circle cx=\"" << cx << "mm\" cy=\"" << cy << "mm\" r=\"" << rad << "mm\" fill=\"black\" stoke-width=\"0\"/>"
              << std::endl;
        }
    }

    const double text_x = 1;
    const double text_y = canvas_height - margin_bottom + std::max(rad0, rad1) * 1e3 + 5;
    f << "<text x=\"" << text_x << "mm\" y=\"" << text_y << "mm\" "
      << "font-size=\"8mm\">"
      << "Distance between circle centers: " << std::fixed << std::setprecision(3) << grid_spacing_ * 1e3 << " mm. "
      << "Long radius: " << std::fixed << std::setprecision(3) << std::max(rad0, rad1) * 1e3 << " mm. "
      << "Short radius: " << std::fixed << std::setprecision(3) << std::min(rad0, rad1) * 1e3 << " mm."
      << "</text>" << std::endl;
