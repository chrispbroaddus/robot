...
void hungarian_print_costmatrix(hungarian_problem_t* p) { hungarian_print_matrix(p->cost, p->num_rows, p->num_cols); }
...
int hungarian_init(hungarian_problem_t* p, double** cost_matrix, int rows, int cols, int mode) {
    ...
    p->row_to_col_map = (int*)calloc(rows, sizeof(int));
    hungarian_test_alloc(p->row_to_col_map);
...
void hungarian_free(hungarian_problem_t* p) {
    ...
    free(p->cost);
    free(p->row_to_col_map);
    p->cost = NULL;
    p->row_to_col_map = NULL;
}
            ...
            for (l = 0; l < n; l++)
                if (slack[l]) {
                    ...
                    if (slack[l] == 0) {
                        ...
                        k = slack_row[l];
                        if (verbose)
                            fprintf(stderr, "Decreasing uncovered elements by %f produces zero at [%d,%d]\n", s, k, l);
                        if (row_mate[l] < 0) {
                            for (j = l + 1; j < n; j++)
                                if (slack[j] == 0)
                                    col_inc[j] += s;
