...
std::string IndentStr(int indent) { return std::string(indent, ' '); }

std::string AttribOpen(const std::string& attrib) { return "<" + attrib + ">"; }

std::string AttribClose(const std::string& attrib) { return "</" + attrib + ">"; }
...
void WriteXmlCamera(std::ostream& out, const std::shared_ptr<CameraInterfaced> cam, int indent) {
    const std::string dd1 = IndentStr(indent);
    const std::string dd2 = IndentStr(indent + 4);

    out << dd1 << "<" << NODE_CAM << " name=\"" << cam->Name() << "\" "
        << "index=\"" << cam->Index() << "\" "
...
std::shared_ptr<CameraInterfaced> ReadXmlCamera(tinyxml2::XMLElement* pEl) {
    ...
    if (sType == "calibu_fu_fv_u0_v0_w") {
        ...
        stream << "Unknown old camera type " << sType << " please implement this"
                                                         " camera before initializing it. ";
...
void WriteXmlCameraAndTransform(std::ostream& out, const std::shared_ptr<CameraInterfaced> cop, int indent) {
...
std::shared_ptr<CameraInterfaced> ReadXmlCameraAndTransform(const std::string& filename) //, const std::shared_ptr<CameraInterfaced> cam)
{
    tinyxml2::XMLDocument doc;
    if (tinyxml2::XML_SUCCESS == doc.LoadFile(filename.c_str())) {
...
std::shared_ptr<Rigd> ReadXmlRig(tinyxml2::XMLNode* xmlrig) {
    ...
    for (tinyxml2::XMLNode* child = xmlrig->FirstChildElement(NODE_CAM_POSE.c_str()); child;
         child = child->NextSiblingElement(NODE_CAM_POSE.c_str())) {
