...
void CreateLookupTable(
    const std::shared_ptr<calibu::CameraInterface<double>>& cam_from, LookupTable& lut, int lookup_width, int lookup_height) {
    /*
    ...
    */
    ...
    R_onKinv << 1.0 / fu, 0, -u0 / fu, 0, 1.0 / fv, -v0 / fv, 0, 0, 1;

    CreateLookupTable(cam_from, R_onKinv, lut, lookup_width, lookup_height);
}

///////////////////////////////////////////////////////////////////////////////
void CreateLookupTable(const std::shared_ptr<calibu::CameraInterface<double>>& cam_from, const Eigen::Matrix3d& R_onKinv, LookupTable& lut,
    int lookup_width, int lookup_height) {
...
void CreateLookupTable(const std::shared_ptr<calibu::CameraInterface<double>>& cam_from, const Eigen::Matrix3d& R_onKinv,
...
///////////////////////////////////////////////////////////////////////////////
void Rectify(const LookupTable& lut, const unsigned char* pInputImageData, unsigned char* pOutputRectImageData, int w, int h) {
    ...
    for (int nRow = 0; nRow < nHeight; nRow++) {
        for (int nCol = 0; nCol < nWidth; nCol++) {
            *pOutputRectImageData++ = (char)(ptr->w00 * pInputImageData[ptr->idx0] + ptr->w01 * pInputImageData[ptr->idx0 + 1]
                + ptr->w10 * pInputImageData[ptr->idx1] + ptr->w11 * pInputImageData[ptr->idx1 + 1]);
            ptr++;
        }
    }
}
