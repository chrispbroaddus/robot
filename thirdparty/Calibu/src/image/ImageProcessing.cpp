...
ImageProcessing::~ImageProcessing() { DeallocateImageData(); }
...
void ImageProcessing::Process(const unsigned char* greyscale_image, size_t w, size_t h, size_t pitch) {
    width = w;
    height = h;
    ...
    // Threshold image
    AdaptiveThreshold(width, height, &I[0], &intI[0], &tI[0], params.at_threshold, width / params.at_window_ratio, 20, (unsigned char)0,
        (unsigned char)255);

    // Label image (connected components)
    labels.clear();
    Label(width, height, &tI[0], &lI[0], labels, params.black_on_white ? 0 : 255);
}
}
