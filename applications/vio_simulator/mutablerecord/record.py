...
class MutableRecord(object):
    ...
    def __init__(self, **kwargs):
        for key, spec in type(self).Fields.items():
    ...
    def __str__(self):
        ...
        """
        fields = ', '.join('%s=%s' % (k, v) for k, v in self.dict.items())
...
class MembersDescriptor(object):
    """
    Gets the map from field names to values for a MutableRecord
    """
    def __init__(self, fieldnames):
        self._fieldnames = fieldnames

    def __get__(self, obj, cls):
        return collections.OrderedDict([(name, getattr(obj, name)) for name in self._fieldnames])
...
class MutableRecordType(type):
    ...
    def __new__(mcl, name, bases, clsdict):
        ...
        namespace['dict'] = MembersDescriptor(fields.keys())
