...
bool populateCalibrationParameters(ConnectionOptions* opts, const calibration::SystemCalibration& calib) {
    bool allFound = true;
    for (uint32_t i = 0; i < (uint32_t)opts->video_sources_size(); i++) {
        std::string name = opts->mutable_video_sources(i)->camera().device().name();
        ...
        for (auto intrinsics : calib.cameraintrinsiccalibration()) {
            if (intrinsics.cameraundercalibration().name() == name) {
                opts->mutable_video_sources(i)->mutable_camera()->mutable_intrinsics()->CopyFrom(intrinsics);
        ...
        for (auto extrinsics : calib.devicetodevicecoordinatetransformation()) {
            if (extrinsics.targetcoordinateframe().device().name() == name) {
                opts->mutable_video_sources(i)->mutable_camera()->mutable_extrinsics()->CopyFrom(extrinsics);
...
bool Connection::FindVideoSource(const std::string& name, VideoSource* video) {
    ...
    if (name == "front") {
        return FindVideoSourceByRole(hal::FrontFisheye, video);
    } else if (name == "rear") {
        return FindVideoSourceByRole(hal::RearFisheye, video);
    }

    return false;
}

bool Connection::FindVideoSourceByRole(hal::CameraId role, VideoSource* video) {
    for (const VideoSource& item : opts_.video_sources()) {
...
void Connection::HandleMessage(websocketpp::connection_hdl h, client_t::message_ptr buf) {
    ...
    if (msg.has_exposure() && exposure_handler_) {
        exposure_handler_(msg.exposure());
    }
