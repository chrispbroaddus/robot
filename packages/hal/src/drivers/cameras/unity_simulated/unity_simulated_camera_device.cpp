...
UnitySimulatedCameraDevice::UnitySimulatedCameraDevice(const uint32_t id, const details::property_map_t& config)
    : m_cameraId(id) {
    ...
    m_cameraSampleSubscriber
        = std::unique_ptr<camera_sample_sub_t>(new camera_sample_sub_t(m_context, publisherAddress, topic, subscriberHighWaterMark));
    LOG(INFO) << "Unity camera subscriber running on ZMQ address: " << publisherAddress << " on topic: " << topic;

    LOG(INFO) << "Unity Simulated Camera created";
}

bool UnitySimulatedCameraDevice::capture(CameraSample& cameraSample) {

    LOG(INFO) << "Capturing Image";
    if (m_cameraSampleSubscriber->poll(std::chrono::milliseconds(m_captureTimeoutInMilliseconds))) {
        if (!m_cameraSampleSubscriber->recv(cameraSample)) {
            LOG(ERROR) << "Capture image failed";
            return false;
        }
    } else {
        LOG(ERROR) << "Capture image timed out";
        return false;
    }
    LOG_EVERY_N(INFO, 10) << "Captured Image";
