...
template <class Derived> inline Eigen::Matrix<typename Derived::Scalar, 3, 3> Skew(const Eigen::MatrixBase<Derived>& vec) {
 ...
 */
template <class Derived> inline Eigen::Matrix<typename Derived::Scalar, 4, 4> Omega(const Eigen::MatrixBase<Derived>& vec) {
    EIGEN_STATIC_ASSERT_VECTOR_SPECIFIC_SIZE(Derived, 3);
    return (Eigen::Matrix<typename Derived::Scalar, 4, 4>() << 0, vec[2], -vec[1], vec[0], -vec[2], 0, vec[0], vec[1], vec[1], -vec[0], 0,
               vec[2], -vec[0], -vec[1], -vec[2], 0)
        .finished();
}
...
/**
 ...
 \return 4x4 Matrix for multiplication with the quaternion
 */
template <class Derived> inline Eigen::Matrix<typename Derived::Scalar, 4, 4> OmegaMatHamilton(const Eigen::MatrixBase<Derived>& vec) {
    EIGEN_STATIC_ASSERT_VECTOR_SPECIFIC_SIZE(Derived, 3);
    return (Eigen::Matrix<typename Derived::Scalar, 4, 4>() << 0, -vec[2], vec[1], vec[0], vec[2], 0, -vec[0], vec[1], -vec[1], vec[0], 0,
               vec[2], -vec[0], -vec[1], -vec[2], 0)
        .finished();
...
/**
 ...
 \endverbatim
 */
template <class Derived> inline Eigen::Matrix<typename Derived::Scalar, 4, 3> XiMat(const Eigen::MatrixBase<Derived>& q_vec) {
...
template <class Derived> Eigen::Quaternion<typename Derived::Scalar> QuaternionFromSmallAngle(const Eigen::MatrixBase<Derived>& theta) {
...
template <class D> bool CheckForNumeric(const Eigen::MatrixBase<D>& mat, const std::string& info) {
    enum { rows = Eigen::MatrixBase<D>::RowsAtCompileTime, cols = Eigen::MatrixBase<D>::ColsAtCompileTime };
