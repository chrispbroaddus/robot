...
public:
    ...
    void HandleVideoRequest(const std::string& conn_id, const Stream& source) {
        ...
        if (it == m_sessions.end()) {
            ...
            it->second->Connect(source.address(), source.topic());
        }
    }
    ...
    void CreateSession(const std::string& conn_id, const Stream& source) {
        ...
        session->Connect(source.address(), source.topic());
        ...
        cricket::VideoCapturer* capturer = new VideoCapturer(session.get(), source.output_width(), source.output_height());
